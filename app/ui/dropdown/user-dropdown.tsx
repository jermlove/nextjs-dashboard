...
export default function UserDropdown() {
  ...
  function toggleDropdown(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
    ...
    if (!isOpen) {
      ...
      const dropdownWidth = 260; // px, matches w-[260px]
      const dropdownHeight = 300; // px, estimated
      ...
      // Default: right-0
      if (spaceRight < 0) {
        // Dropdown would overflow right, shift left
        style.right = Math.max(0, spaceRight - 8) + 'px';
      } else if (spaceLeft < dropdownWidth - rect.width) {
        // Dropdown would overflow left, shift right
        style.left = '0px';
      }
    ...
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11 flex items-center justify-center bg-gray-100 dark:bg-gray-800">
          {image ? (
            <Image width={44} height={44} src={image} alt={name} />
          ) : (
            // Lucide User Icon (default)
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400 dark:text-gray-500">
      ...
      <Dropdown
        ...
        className={`absolute flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark ${dropUp ? 'mb-[17px] bottom-full' : 'mt-[17px] top-full'}`}
        style={dropdownStyle}
      >
        <div>
